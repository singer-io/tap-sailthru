...
class SailthruStartDateTest(SailthruBaseTest):

    """
    Ensure both all expected streams respect the start date. Run tap in check mode, 
    run 1st sync with start date = 2021-04-01, run check mode and 2nd sync on a new connection with start date = 10 days later.
    """
    ...
    def name(self):
        return "sailthru_start_date_test"
    ...
    def run_test(self, expected_streams):
        ...
        self.start_date_2 = self.timedelta_formatted(self.start_date_1, days=10)
