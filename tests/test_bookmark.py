...
class SailthruBookmarkTest(SailthruBaseTest):
    """Test tap sets a bookmark and respects it for the next sync of a stream"""
    
    def name(self):
        return "sailthru_bookmark_test"
                ...
                if expected_replication_method == self.INCREMENTAL:
                    ...
                    for record in first_sync_messages:
                        ...
                        self.assertLessEqual(
                            replication_key_value, first_bookmark_value.get(replication_key),
                    ...
                    for record in second_sync_messages:
                        ...
                        self.assertGreaterEqual(replication_key_value, new_states['bookmarks'][stream].get(replication_key),
                                                msg="Second sync records do not repect the previous bookmark.")

                        # Verify the second sync bookmark value is the max replication key value for a given stream
                        self.assertLessEqual(
                            replication_key_value, second_bookmark_value.get(replication_key),
